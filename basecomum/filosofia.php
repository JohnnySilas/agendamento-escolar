...
<?php
 include("conexao.php");
  ...
  if(isset($_GET['deletar'])){
    ...
    $sql_query = $mysqli->query("SELECT * FROM filosofia WHERE id = '$id'") or die($mysqli->error);
    $arquivo1 = $sql_query->fetch_assoc();

    if(unlink($arquivo1['path'])) {
      $deu_certo = $mysqli->query("DELETE FROM filosofia WHERE id = '$id'") or die($mysqli->error);
    }
 ...
 function enviarArquivo($name, $tmp_name){
   ...
   if($extensao != 'rar' && $extensao != 'zip')
     ...
     if ($deu_certo) { 
       $mysqli->query("INSERT INTO filosofia (nome, path) VALUES('$nomeDoArquivo', '$path')") or die($mysqli->error);
       return true;
 }   else
       return false;
 ...
 if(isset($_FILES['filosofia'])) {
   $arquivos = $_FILES['filosofia'];
   $tudo_certo = true;
   foreach($arquivos['name'] as $filosofia => $arq){
    $deu_certo = enviarArquivo($arquivos['name'][$filosofia], $arquivos["tmp_name"][$filosofia]);
    if(!$deu_certo)
         $tudo_certo = false;
   }
...
include("conexao.php");
 ...
 if(isset($_GET['deletarr'])){
   ...
   $mysql_query = $mysqli->query("SELECT * FROM filosofia2 WHERE id = '$id'") or die($mysqli->error);
   $arquivo = $mysql_query->fetch_assoc();

   if(unlink($arquivo['path'])) {
     $deu_certo = $mysqli->query("DELETE FROM filosofia2 WHERE id = '$id'") or die($mysqli->error);
   }
...
function enviarArquivos($name, $tmp_name){
  ...
  if($extensao != 'rar' && $extensao != 'zip')
    ...
    $deu_certo = move_uploaded_file($tmp_name, $path);
    if ($deu_certo) { 
      $mysqli->query("INSERT INTO filosofia2 (nome, path) VALUES('$nomeDoArquivo', '$path')") or die($mysqli->error);
      return true;
}   else
      return false;
...
if(isset($_FILES['filosofia2'])) {
  $arquivos = $_FILES['filosofia2'];
  $tudo_certo = true;
  foreach($arquivos['name'] as $filosofia2 => $arq){
   $deu_certo = enviarArquivos($arquivos['name'][$filosofia2], $arquivos["tmp_name"][$filosofia2]);
   if(!$deu_certo)
        $tudo_certo = false;
  }
...
<div class="list-group" style="border-bottom: 1px solid black; border-top: 1px  solid black;">
  <a href="#" class="list-group-item list-group-item-action">O QUE É FILOSOFIA ?</a>
  <a href="#" class="list-group-item list-group-item-action">TEORIA DO CONHECIMENTO</a>
  <a href="#" class="list-group-item list-group-item-action">ÉTICA</a>
</div>
<div class="container-fluid" style="background-color:#545454; margin-top: 25px; color: white;">
...
<table>
  ...
  <td class="list-group-item list-group-item-action"><a href="filosofia.php?deletar=<?php echo $arquivo1['id']; ?>">Deletar</a></td>
</tr>
<?php
}
?>
...
<div class="mt-0" style="width: 145px;">
  ...
  <input class="form-control" multiple type="file" id="formFile" name="filosofia[]">
...
<table>
  ...
  <td class="list-group-item list-group-item-action"><a href="filosofia.php?deletarr=<?php echo $arquivo['id']; ?>">Deletar</a></td>
</tr>
<?php
}
?>
...
<div class="mt-0" style="width: 145px;">
  <form method="post" enctype="multipart/form-data">
  <label for="formFile" class="form-label"></label>
  <input class="form-control" multiple type="file" id="formFile" name="filosofia2[]">
  <input class="mt-2" type="submit" name="upload" value="Enviar">
</div>
</form>
