...
<?php
 include("conexao.php");
  ...
  if(isset($_GET['deletar'])){
    ...
    $sql_query = $mysqli->query("SELECT * FROM arte WHERE id = '$id'") or die($mysqli->error);
    $arquivo1 = $sql_query->fetch_assoc();

    if(unlink($arquivo1['path'])) {
      $deu_certo = $mysqli->query("DELETE FROM arte WHERE id = '$id'") or die($mysqli->error);
    }
 ...
 function enviarArquivo($name, $tmp_name){
   ...
   if($extensao != 'rar' && $extensao != 'zip')
     ...
     if ($deu_certo) { 
       $mysqli->query("INSERT INTO arte (nome, path) VALUES('$nomeDoArquivo', '$path')") or die($mysqli->error);
       return true;
 }   else
       return false;
 ...
 if(isset($_FILES['arte'])) {
   $arquivos = $_FILES['arte'];
   $tudo_certo = true;
   foreach($arquivos['name'] as $arte => $arq){
    $deu_certo = enviarArquivo($arquivos['name'][$arte], $arquivos["tmp_name"][$arte]);
    if(!$deu_certo)
         $tudo_certo = false;
   }
 ...
 $sql_query = $mysqli->query("SELECT * FROM arte") or die($mysqli->error);
...
include("conexao.php");
 ...
 if(isset($_GET['deletarr'])){
   ...
   $mysql_query = $mysqli->query("SELECT * FROM arte2 WHERE id = '$id'") or die($mysqli->error);
   $arquivo = $mysql_query->fetch_assoc();

   if(unlink($arquivo['path'])) {
     $deu_certo = $mysqli->query("DELETE FROM arte2 WHERE id = '$id'") or die($mysqli->error);
   }
...
function enviarArquivos($name, $tmp_name){
  ...
  if($extensao != 'rar' && $extensao != 'zip')
    ...
    $deu_certo = move_uploaded_file($tmp_name, $path);
    if ($deu_certo) { 
      $mysqli->query("INSERT INTO arte2 (nome, path) VALUES('$nomeDoArquivo', '$path')") or die($mysqli->error);
      return true;
}   else
      return false;
...
if(isset($_FILES['arte2'])) {
  $arquivos = $_FILES['arte2'];
  $tudo_certo = true;
  foreach($arquivos['name'] as $arte => $arq){
   $deu_certo = enviarArquivos($arquivos['name'][$arte], $arquivos["tmp_name"][$arte]);
   if(!$deu_certo)
        $tudo_certo = false;
  }
...
$mysql_query = $mysqli->query("SELECT * FROM arte2") or die($mysqli->error);
...
<div class="list-group" style="border-bottom: 1px solid black; border-top: 1px  solid black;">
  <a href="#" class="list-group-item list-group-item-action">ELEMENTOS DA LINGUAGEM</a>
  <a href="#" class="list-group-item list-group-item-action">BARROCO</a>
  <a href="#" class="list-group-item list-group-item-action">PATRIMÔNIO CULTURAL</a>
</div>
<div class="container-fluid" style="background-color:#c4c415; margin-top: 25px; color: white;">
...
<table>
  ...
  <td class="list-group-item list-group-item-action"><a href="arte.php?deletar=<?php echo $arquivo1['id']; ?>">Deletar</a></td>
</tr>
<?php
}
?>
...
<div class="mt-0" style="width: 145px;">
  ...
  <input class="form-control" multiple type="file" id="formFile" name="arte[]">
...
<table>
  ...
  <td class="list-group-item list-group-item-action"><a href="arte.php?deletarr=<?php echo $arquivo['id']; ?>">Deletar</a></td>
</tr>
<?php
}
?>
...
<div class="mt-0" style="width: 145px;">
  <form method="post" enctype="multipart/form-data">
  <label for="formFile" class="form-label"></label>
  <input class="form-control" multiple type="file" id="formFile" name="arte2[]">
  <input class="mt-2" type="submit" name="upload" value="Enviar">
</div>
</form>
