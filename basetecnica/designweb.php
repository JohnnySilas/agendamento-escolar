...
<?php
 include("conexao.php");
  ...
  if(isset($_GET['deletar'])){
    ...
    $sql_query = $mysqli->query("SELECT * FROM designweb WHERE id = '$id'") or die($mysqli->error);
    $arquivo1 = $sql_query->fetch_assoc();

    if(unlink($arquivo1['path'])) {
      $deu_certo = $mysqli->query("DELETE FROM designweb WHERE id = '$id'") or die($mysqli->error);
    }
 ...
 function enviarArquivo($name, $tmp_name){
   ...
   if($extensao != 'rar' && $extensao != 'zip')
     ...
     if ($deu_certo) { 
       $mysqli->query("INSERT INTO designweb (nome, path) VALUES('$nomeDoArquivo', '$path')") or die($mysqli->error);
       return true;
 }   else
       return false;
 ...
 if(isset($_FILES['designweb'])) {
   $arquivos = $_FILES['designweb'];
   $tudo_certo = true;
   foreach($arquivos['name'] as $designweb => $arq){
    $deu_certo = enviarArquivo($arquivos['name'][$designweb], $arquivos["tmp_name"][$designweb]);
    if(!$deu_certo)
         $tudo_certo = false;
   }
 ...
 $sql_query = $mysqli->query("SELECT * FROM designweb") or die($mysqli->error);
...
include("conexao.php");
 ...
 if(isset($_GET['deletarr'])){
   ...
   $mysql_query = $mysqli->query("SELECT * FROM designweb2 WHERE id = '$id'") or die($mysqli->error);
   $arquivo = $mysql_query->fetch_assoc();

   if(unlink($arquivo['path'])) {
     $deu_certo = $mysqli->query("DELETE FROM designweb2 WHERE id = '$id'") or die($mysqli->error);
   }
...
function enviarArquivos($name, $tmp_name){
  ...
  if($extensao != 'rar' && $extensao != 'zip')
    ...
    $deu_certo = move_uploaded_file($tmp_name, $path);
    if ($deu_certo) { 
      $mysqli->query("INSERT INTO designweb2 (nome, path) VALUES('$nomeDoArquivo', '$path')") or die($mysqli->error);
      return true;
}   else
      return false;
...
if(isset($_FILES['designweb2'])) {
  $arquivos = $_FILES['designweb2'];
  $tudo_certo = true;
  foreach($arquivos['name'] as $designweb => $arq){
   $deu_certo = enviarArquivos($arquivos['name'][$designweb], $arquivos["tmp_name"][$designweb]);
   if(!$deu_certo)
        $tudo_certo = false;
  }
...
$mysql_query = $mysqli->query("SELECT * FROM designweb2") or die($mysqli->error);
...
<nav class="navbar navbar-expand-md  navbar-dark justify-content-end" style="background-color: #c4c415; font-family: Cairo">
  <div class="container-fluid">
	...
	<section>
		...
		<h1>Desenvolvimento e Design de WebSites</h1>
	</div>
</section>
...
<table>
  ...
  <td class="list-group-item list-group-item-action"><a href="designweb.php?deletar=<?php echo $arquivo1['id']; ?>">Deletar</a></td>
</tr>
<?php
}
?>
...
<div class="mt-0" style="width: 145px;">
  ...
  <input class="form-control" multiple type="file" id="formFile" name="designweb[]">
...
<table>
  ...
  <td class="list-group-item list-group-item-action"><a href="designweb.php?deletarr=<?php echo $arquivo['id']; ?>">Deletar</a></td>
</tr>
<?php
}
?>
...
<div class="mt-0" style="width: 145px;">
  <form method="post" enctype="multipart/form-data">
  <label for="formFile" class="form-label"></label>
  <input class="form-control" multiple type="file" id="formFile" name="designweb2[]">
  <input class="mt-2" type="submit" name="upload" value="Enviar">
</div>
</form>
