...
<?php
 include("conexao.php");
  ...
  if(isset($_GET['deletar'])){
    ...
    $sql_query = $mysqli->query("SELECT * FROM opsoft WHERE id = '$id'") or die($mysqli->error);
    $arquivo1 = $sql_query->fetch_assoc();

    if(unlink($arquivo1['path'])) {
      $deu_certo = $mysqli->query("DELETE FROM opsoft WHERE id = '$id'") or die($mysqli->error);
    }
 ...
 function enviarArquivo($name, $tmp_name){
   ...
   if($extensao != 'rar' && $extensao != 'zip')
     ...
     if ($deu_certo) { 
       $mysqli->query("INSERT INTO opsoft (nome, path) VALUES('$nomeDoArquivo', '$path')") or die($mysqli->error);
       return true;
 }   else
       return false;
 ...
 if(isset($_FILES['opsoft'])) {
   $arquivos = $_FILES['opsoft'];
   $tudo_certo = true;
   foreach($arquivos['name'] as $opsoft => $arq){
    $deu_certo = enviarArquivo($arquivos['name'][$opsoft], $arquivos["tmp_name"][$opsoft]);
    if(!$deu_certo)
         $tudo_certo = false;
   }
 ...
 $sql_query = $mysqli->query("SELECT * FROM opsoft") or die($mysqli->error);
...
include("conexao.php");
 ...
 if(isset($_GET['deletarr'])){
   ...
   $mysql_query = $mysqli->query("SELECT * FROM opsoft2 WHERE id = '$id'") or die($mysqli->error);
   $arquivo = $mysql_query->fetch_assoc();

   if(unlink($arquivo['path'])) {
     $deu_certo = $mysqli->query("DELETE FROM opsoft2 WHERE id = '$id'") or die($mysqli->error);
   }
...
function enviarArquivos($name, $tmp_name){
  ...
  if($extensao != 'rar' && $extensao != 'zip')
    ...
    $deu_certo = move_uploaded_file($tmp_name, $path);
    if ($deu_certo) { 
      $mysqli->query("INSERT INTO opsoft2 (nome, path) VALUES('$nomeDoArquivo', '$path')") or die($mysqli->error);
      return true;
}   else
      return false;
...
if(isset($_FILES['opsoft2'])) {
  $arquivos = $_FILES['opsoft2'];
  $tudo_certo = true;
  foreach($arquivos['name'] as $opsoft => $arq){
   $deu_certo = enviarArquivos($arquivos['name'][$opsoft], $arquivos["tmp_name"][$opsoft]);
   if(!$deu_certo)
        $tudo_certo = false;
  }
...
$mysql_query = $mysqli->query("SELECT * FROM opsoft2") or die($mysqli->error);
...
<nav class="navbar navbar-expand-md  navbar-dark justify-content-end" style="background-color: #800080; font-family: Cairo">
  <div class="container-fluid">
	...
	<section>
		...
		<h1>Operações de Software Aplicativo</h1>
	</div>
</section>
...
<div class="list-group" style="border-bottom: 1px solid black; border-top: 1px  solid black;">
  <a href="#" class="list-group-item list-group-item-action">METODOLOGIA SCRUM</a>
  <a href="#" class="list-group-item list-group-item-action">SCRUM.1</a>
  <a href="#" class="list-group-item list-group-item-action">SCRUM.2</a>
</div>
<div class="container-fluid" style="background-color:#800080; margin-top: 25px; color: white;">
...
<table>
  ...
  <td class="list-group-item list-group-item-action"><a href="opsoft.php?deletar=<?php echo $arquivo1['id']; ?>">Deletar</a></td>
</tr>
<?php
}
?>
...
<div class="mt-0" style="width: 145px;">
  ...
  <input class="form-control" multiple type="file" id="formFile" name="opsoft[]">
...
<table>
  ...
  <td class="list-group-item list-group-item-action"><a href="opsoft.php?deletarr=<?php echo $arquivo['id']; ?>">Deletar</a></td>
</tr>
<?php
}
?>
...
<div class="mt-0" style="width: 145px;">
  <form method="post" enctype="multipart/form-data">
  <label for="formFile" class="form-label"></label>
  <input class="form-control" multiple type="file" id="formFile" name="opsoft2[]">
  <input class="mt-2" type="submit" name="upload" value="Enviar">
</div>
</form>
